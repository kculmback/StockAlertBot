...
export default async function argos(url, interval) {
  try {
    ...
    if (res && res.status == 200) {
      ...
      let title = doc.getElementsByClassName(
        'Namestyles__Main-sc-269llv-1 bojEI'
      );
      let inventory = doc.getElementsByClassName(
        'Buttonstyles__Button-q93iwm-2 dUQXJf'
      );
      let image = doc.getElementsByClassName(
        'MediaGallerystyles__ImageWrapper-sc-1jwueuh-2 bhjltf'
      );

      if (title.length > 0)
        title = title[0].firstChild.textContent.trim().slice(0, 150);
      else {
